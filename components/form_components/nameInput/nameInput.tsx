...
const NameInput = ({register, error, inputTitle}) => {
  const namePart = _.upperFirst(inputTitle === 'firstName'?'first name':'last name')
  return (
    <div>
        <div className="relative h-10 w-full min-w-[200px]">
          <input
            ...
            {...register(inputTitle,{
              ...
              pattern:{
                value: /^.{2,20}$/,
                message:`${namePart} must be 2-20 characters`
              },
              validate:{
                notAdmin: (fieldValue:string) =>{
                  return(
                    fieldValue !== "admin" ||
                    `Enter a diffrent ${namePart}`
                  )
                },
              }
            })}
